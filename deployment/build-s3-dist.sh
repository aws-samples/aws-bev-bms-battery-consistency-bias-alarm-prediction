...
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
    ...
    echo "For example: ./build-s3-dist.sh solutions trademark_approved_solution_name v1.0.0"
    exit 1
fi
...
# Move sample data to template_dist_dir
echo "Move sample dataset and script from sagemaker directory to template_dist_dir"
echo "cp $template_dir/../source/sagemaker/sample-data/series_samples.csv $template_dist_dir/"
cp $template_dir/../source/sagemaker/sample-data/series_samples.csv $template_dist_dir/
cp $template_dir/../source/sagemaker/xgboost_beginner.ipynb $template_dist_dir/
...
for d in `find . -mindepth 1 -maxdepth 1 -type d`; do
    ...
    if ls $fname/*.py 1> /dev/null 2>&1; then
        ...
        mkdir $fname
        mv *.py $fname
        zip -gq $staging_dist_dir/$fname.zip $fname/*
        cd $staging_dist_dir
